...
\submitdate{March 13\\(For Graduation May 2017)}        
...
%TODO(meeting mar10): remove xiaotian cluster picture for now
    % explain technologies more, e.g. docker, spark streaming, mapReduce, micro-batch
%MINIMUM REQUIREMENTS BEFORE SENDING: reference EVERY FIGURE, intro, abstract, beginning of section 3 w/paragraph about vehicles, clusterheads, cloud, v2v - mention we assume vehicles have some capability (give example connections - 3g/4g, DSRC); put conclusion in past-tense 
%TODO: guideline - have someone outside of CS read introduction- they should be able to get the gist of what is going on
%WHEN SENDING: send pdf, can run by patterson first; mention that sending draft and may send small edits
...
\specialhead{ABSTRACT}
    ...
    produce bandwidth savings of over 75\% compared to all vehicles transferring data to the cloud.
     %TODO: reference implementation that does no map or reduce (maybe collect message size counts for sensorDaemon messages at each vehicle with nullable new results column; filter not null results to calculate bandwidth savings
    %TODO(mar 10): have reference run with no map or reduce (reduce nop, self clusters)
...
\chapter{INTRODUCTION}
    ...
    % Motivate why we want to get data from vehicles to cloud, and why it's hard
    ...
    %TODO (Mar 15): Define V2I (vehicle to INFRASTRUCTURE) which is how we would get data from vehicles to cloud
    %TODO (Mar 15): need to analyize data, state of art is to analyze it in the cloud; limited bandwidth means vehicle-to-cloud connectivity not adequate for sending all data to the cloud...

    % How we get around limitations of getting data into the cloud
    ...
    % TODO (mar 15): \cite{sbca}\cite{cdrive}\cite{clusterselection}\cite{clustering:mobaware}.
    % these vanets can be used for forming clusters with clustersheads, use cases include (list from citations): ...
    % we wish to use clusters to perform aggregation inside clusters so as to reduce amount of bandwidth between vehicles and infrastructure
    % MAKE SURE WE DEFINE CLUSTERHEAD

    %TODO (mar 15): pick a convention for clusterheads and stick with it (one word, consistent case)
   ...
   % between resolution and latency versus bandwidth consumption. 
   % Having a smaller time window for reduction means
   ...
   % has an effect on overall bandwidth usage.
    %TODO(mar 10): move discussion of reduce window trade-offs w.r.t. resolution and latency to experimental results or conclusion; also define latency/resolution (maybe give example)

    The major contributions of the work are:
    %TODO (mar 15): these should all be complete sentences
        \begin{itemize}
            ...
            \item The framework integrates with Spark Streaming to allow the trivial creation of edge computing queries that are ingested into Spark Streaming. %TODO (Mar 15): mention this is automated
            \item Actor-model-based simulator for map/reduce queries on dynamic clusters of connected vehicles.
                ...
                nodes.
            \item %TODO (Mar 15): an evaluation of the framework using realistic trace data with various parameters
        \end{itemize}
    ...
    %TODO: Papers don't do work. They present things.
...
\chapter{Background and Related Work}
    ...
    \subsection{Vehicle Clusters}
        \paragraph{Clustering Algorithms}
            ...
            is an area of active research \cite{cdrive}\cite{sbca}\cite{clusterselection}.%TODO: fix citation \cite{clustering:mobware}.
    ...
    \subsection{Spark Streaming}
        ...
        %TODO(meeting mar10): define micro-batch, give examples of map(), filter(), reduce()
    ...
    \subsection{Cloud IoT Hubs}
        ...
        \paragraph{Other IoT Platforms}
            ...
            \cite{forbes:iot}. %TODO: relate these to Spindle; say whether they are edge computing frameworks
...
\chapter{System Architecture}
    ...
    sends aggregated data to the cloud for further processing. %TODO(mar 15): flesh out description of what is going on in figure
...
\subsection{Component Interactions}
    Figure \ref{fig:theoretical:sequence} is a sequence diagram showing how queries propagate to vehicles and how data passes to the cloud. %TODO: spark to middleware, middleware to clusterheads/vehicle,s vehicles to clusterheads, ..., back to spark programk
   %TODO(mar 15): add sentences describing who is talking to who in sequence diagram
...
\subsection{Vehicle Traces}
   ...
   %TODO(meeting mar10): this is a surprising result that ... for speedSum and geoMapped, give intuition for why this is case (small number of geoMap regions so each cluster probably has messages for only one region, therefore growth in data consumption is not large);  results dominated by messages from indivudal vehicles vs multiple messages per clusterhead
   %TODO(meeting mar10): say explicitly that we care about v2I bandwidth being measured (say this repeatedly), say we are less concerned about v2v bandwidth consumption; also add that we have single-hop clusters

\subsubsection{Test Regions} %TODO(MEETING): make sub-sub section of Vehicle traces subsection
    \paragraph{Original VEINS Traces} %TODO(mar 15): fix formatting (may have to rephrase sentence)
    ...
    %    \caption{Illustration of VEINS Vehicles with MANET Clustering, Courtesy Duan \cite{xiaotian}}
    %    \label{fig:clusters}
    %\end{figure}
    \begin{figure}
        \centering
...
\subsection{Test Clusters}
    ...
    %TODO(mar 10): self clusters behaves as if vehicles didn't have clustering ability
...
\section*{Future Work}
    ...
    %TODO( mar 15): too much 'improve', too much 'opportunities'
    to improve cluster size and stability by integrating available information. 
    %TODO( Mar 15): remove the 'the topic of' sentence; just rephrase 
    %The topic of what useful information can be gathered from connected vehicles will likely remain a perpetually open question,
    ...
    %from traffic flows based on variation in travel among seasons (inicating which areas host people with full
    %time versus seasonal jobs). The real-time element might also allow real-time response to potential accidents
    %and obstructions based on live traffic data. 
    %TODO: Mar 15: because there is so much data...
    %SUGGESTION: make a bulleted list of future work ideas
