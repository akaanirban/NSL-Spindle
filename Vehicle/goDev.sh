...
docker run --rm -d --name $DOCKER_NAME -v $HOME/.ivy2:/root/.ivy2 -v `pwd`:/root/dev/vehicle -v `cd ../Shared && pwd`:/root/dev/Shared -e DEBUG_VEHICLE=true -p9001:9001 nslrpi/spindle-node:latest
